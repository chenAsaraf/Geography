...
public class Point3D implements Geom_element, Serializable {
	...
	/**
	 * Return the x point value as an integer
	 * @return _x int 
	 */
	...
	/**
	 * Return the y point value as an integer
	 * @return _y int
	 */
	public int iy() {return (int)_y;}

	/**
	 * Return the z point value as an integer
	 * @return _z int
	 */
	...
	/**
	 * Add the Point values ​​given to the current point
	 * @param p Point3D
	 */
	...
	/**
	 * Add the values ​​given to the current values point
	 * @param dx double 
	 * @param dy double 
	 * @param dz double
	 */
	...
	public double angleXY_2PI(Point3D p) 
	{
		if(p == null) throw new RuntimeException("** Error: Point3D angle got null **");
		double angleXY = Math.atan2((p._y-_y), (p._x-_x));
		if (angleXY < 0 ) angleXY = 2 * Math.PI + angleXY;
		return angleXY;
	}
	...
	public double angleZ(Point3D p) 
	{
		...
		double angleZ = Math.atan2((p._z - this._z), this.distance2D(p));
		return angleZ;
	}	


	/** return the planer angle of the vector between this point to other point p,
	 * in DEGREES, in a compass order: north 0, east 90, south 180, west 270.
	...
	/** return the vertical angles in DEGREES of the vector this point to other point p
	 * @param p Point3D
	 * @return an in degrees 
	 * */
	...
	/** return the vertical angles in DEGREES of the vector from this point to other point p, 
	 *  @param height: is the extra height of the point p (used by GISElement).
	 *  @return ans in degrees 
	 * */
	public double up_angle(Point3D p, double height)
	{
		double ans = 0;
		ans = Math.atan2((p._z + height - this._z), this.distance2D(p));
		return Math.toDegrees(ans);
	}
